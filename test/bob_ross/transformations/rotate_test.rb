...
class BobRossTransformationsRotateTest < Minitest::Test
  ...
  test 'rotating an image' do
    ...
    assert_transform(image, {rotate: 8}, { geometry: '780x576' })
    assert_transform(image, {rotate: 45}, { geometry: '849x849' })
    assert_transform(image, {rotate: 90}, { geometry: '480x720' })
    assert_transform(image, {rotate: 135}, { geometry: '849x849' })
    assert_transform(image, {rotate: 180}, { geometry: '720x480' })
    assert_transform(image, {rotate: 225}, { geometry: '849x849' })
    assert_transform(image, {rotate: 270}, { geometry: '480x720' })
    assert_transform(image, {rotate: 315}, { geometry: '849x849' })
